...
> 需要了解的主要配置：`title tooltip legend toolbox grid xAxis yAxis`     
...
- grid：直角坐标系内绘图网格。 可控制大小

- title：标题组件

- tooltip：提示框组件

- legend：图例组件(上方表示每个线表示什么)

- toolbox： 工具箱组件(可下载成图片等功能 )
...
option = {
    ...
    // 图例组件
    legend: {
       // **如果series里面有了 name值则 legend里面的data可以删掉
    },
